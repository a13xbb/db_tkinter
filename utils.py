...
def flush_table(mode, engine):
    conn = engine.connect()
    if mode == '*':
        conn.execute(f'TRUNCATE TABLE item CASCADE;')
        conn.execute(f'TRUNCATE TABLE purchase_item CASCADE;')
        conn.execute(f'TRUNCATE TABLE purchase CASCADE;')
        conn.execute(f'TRUNCATE TABLE transaction CASCADE;')
    else:
